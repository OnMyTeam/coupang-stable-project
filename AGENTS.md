## 개발자 커피타임 보존 AI Agent

이 저장소에서 Codex는 Jira/Confluence PRD 업데이트를 기준으로 SmartContract와 Blockchain-API 변경사항을 분석하고, 승인 단계를 거쳐 구현, 검증, 배포 안내까지 진행한다.

### 기본 원칙

- 모든 Jira 또는 Confluence 관련 문서 확인 요청은 반드시 Jira MCP를 사용한다.
- Notion은 어떤 단계에서도 사용하지 않는다.
- Jira MCP가 현재 세션에서 사용할 수 없으면 웹 검색이나 추측으로 대체하지 말고, Jira MCP 연결이 필요하다고 사용자에게 알린다.
- SmartContract 주 분석 대상은 `smartcontract/src/Payment.sol`이다. 사용자가 `smartcontract/src/payment.sol`로 지칭해도 동일 파일로 해석한다.
- Blockchain-API 주 분석 대상은 `blockchain-api/src/server.js`, `blockchain-api/src/paymentAbi.js`, `blockchain-api/public/*`, `blockchain-api/deployments/payment.json`이다.
- 로컬 명령 실행과 검증은 기본적으로 Git Bash에서 작업하는 것을 기준으로 안내하고 수행한다.
- Windows/PowerShell 전용 명령이 필요하거나 Git Bash에서 실행할 수 없는 경우에만 PowerShell을 사용하고, 그 이유를 보고한다.
- 기존 사용자 변경사항을 되돌리지 않는다.

### PRD 업데이트 분석 트리거

사용자가 다음 형식으로 요청하면 PRD 업데이트 분석 워크플로를 시작한다.

```text
현재 {{JIRA Page}} PRD 업데이트 내용 분석해서 SmartContract와 Blockchain-API 수정사항 알려줘
```

진행 순서:

1. Jira MCP로 `{{JIRA Page}}`의 최신 PRD/Confluence 내용을 확인한다.
2. SmartContract와 Blockchain-API에 영향을 주는 요구사항만 추출한다.
3. 분석 결과를 `prd_review/{{PRD_pageName}}_ver1.txt`에 저장한다.
4. 같은 PRD에 대해 새 버전이 필요하면 기존 파일을 확인한 뒤 `ver2`, `ver3`처럼 증가시킨다.

`prd_review` 파일 형식:

```text
# {{PRD_pageName}} {{version}} PRD 분석

## 문서 정보
- Jira Page: {{JIRA Page}}
- 분석 일시: {{YYYY-MM-DD HH:mm KST}}
- 대상 영역: SmartContract, Blockchain-API

## PRD 핵심 변경사항
1. {{변경사항 요약}}

## SmartContract 영향
- {{영향 및 근거}}

## Blockchain-API 영향
- {{영향 및 근거}}

## 확인 필요 사항
- {{모호하거나 사용자 확인이 필요한 항목}}
```

### 역할 에이전트

PRD 분석 파일이 만들어지면 다음 두 역할을 기준으로 변경 리스트를 작성한다.

- Contract_Agent: `.agents/Contract_Agent.md`
- Blockchain-API_Agent: `.agents/Blockchain-API_Agent.md`

두 역할의 분석 결과를 합쳐 `list/{{PRD_pageName}}_{{version}}.json`에 저장한다.
각 리스트 항목은 SmartContract와 Blockchain-API의 변경 전/후를 각각 분리해서 기록한다.
둘 중 한 영역에 수정이 필요 없으면 해당 영역의 `before`와 `after`에 `수정사항 없음`을 기록한다.

리스트 JSON 형식:

```json
{
  "prdPageName": "{{PRD_pageName}}",
  "version": "{{version}}",
  "sourceReview": "prd_review/{{PRD_pageName}}_{{version}}.txt",
  "items": [
    {
      "number": 1,
      "summary": "{{변경할 내용 요약}}",
      "smartContract": {
        "before": "{{SmartContract 변경전 내용 또는 수정사항 없음}}",
        "after": "{{SmartContract 변경후 내용 또는 수정사항 없음}}"
      },
      "blockchainApi": {
        "before": "{{Blockchain-API 변경전 내용 또는 수정사항 없음}}",
        "after": "{{Blockchain-API 변경후 내용 또는 수정사항 없음}}"
      },
      "status": "pending"
    }
  ]
}
```

### 승인 및 개발 진행

`list/{{PRD_pageName}}_{{version}}.json` 작성 후 번호 순서대로 하나씩 사용자 채팅과 Slack에 알린다.

- Slack 알림은 `#stable-project-codex` 채널로 전달한다.
- 채널 ID가 필요한 경우 `C0BG04CV3FD`를 사용한다.

알림 형식:

```text
{{번호}}. {{변경할 내용 요약}}
- SmartContract 변경전: {{내용 또는 수정사항 없음}}
- SmartContract 변경후: {{내용 또는 수정사항 없음}}
- Blockchain-API 변경전: {{내용 또는 수정사항 없음}}
- Blockchain-API 변경후: {{내용 또는 수정사항 없음}}

응답: 1:진행, 2:스킵, 3:코멘트
```

진행 규칙:

- 사용자가 알림 또는 list 처리 개수를 명시하면 해당 개수를 이번 요청의 개발 완료 기준 개수로 삼는다.
- 개발 완료 기준 개수만큼 유효 응답을 받고, 각 항목이 `completed` 또는 `skipped`로 처리되면 이번 개발은 완료된 것으로 본다.
- 사용자가 "N개 모두 응답이 되면 개발 완료" 또는 "N개 모두 응답이 되면 개발이 완료된 것이다"처럼 완료 조건을 명시해도 최종 리포트, 배포 명령, GitHub Push 확인은 기존 규칙대로 자동 진행한다.
- 각 번호는 `1`, `2`, `3` 중 하나의 유효 응답을 받은 뒤에만 다음 번호로 넘어간다.
- 각 승인 질문은 반드시 현재 Codex 채팅창과 Slack에 동시에 알린다. Slack MCP가 사용 가능하면 Slack thread와 `#stable-project-codex` 채널 본문을 모두 응답 확인 대상으로 삼는다.
- 승인 질문을 보낸 뒤 최대 1분 동안 현재 Codex 채팅창과 Slack 양쪽의 응답을 기다리고 확인한다.
- 1분 안에 현재 Codex 채팅창, Slack thread, 또는 `#stable-project-codex` 채널 본문 중 한 곳에서 `1`, `2`, `3` 또는 `1:진행`, `2:스킵`, `3:코멘트` 형식의 유효 응답이 확인되면 해당 응답대로 처리한다.
- 1분 안에 유효 응답이 없으면 다음 번호로 넘어가지 않고, 구현도 진행하지 않으며, 해당 번호가 승인 대기 중임을 사용자에게 보고한다.
- 사용자 채팅 응답, Slack thread 응답, Slack 채널 본문 응답은 동일한 승인 입력으로 처리한다.
- Slack MCP가 사용 가능하면 승인 알림을 보낸 뒤 최대 1분 동안 해당 thread와 `#stable-project-codex` 채널의 최신 사용자 응답을 확인한다. 단, 1분 안에 유효 응답이 확인되면 즉시 해당 선택지대로 진행할 수 있다.
- 사용자가 "Slack에 응답했다", "슬랙 확인해", "응답했어"처럼 말하면 즉시 Slack MCP로 thread와 채널 본문을 읽고 최신 유효 응답을 찾아야 한다.
- Slack에서 `1`, `2`, `3` 또는 `1:진행`, `2:스킵`, `3:코멘트` 형식의 응답이 오면 채팅 응답을 받은 것과 동일하게 계속 진행한다.
- Slack과 채팅 양쪽에 응답이 모두 있는 경우 먼저 확인한 유효 응답을 기준으로 처리하고, 이후 충돌 응답이 보이면 사용자에게 확인한다.
- Slack 응답이 `1` 또는 `1:진행`이면 별도 재확인 없이 해당 변경사항을 구현하고 리스트 상태를 `completed`로 기록한다.
- Slack 응답이 `2` 또는 `2:스킵`이면 별도 재확인 없이 구현하지 않고 리스트 상태를 `skipped`로 기록한다.
- Slack 응답이 `3` 또는 `3:코멘트`이면 Slack의 코멘트 내용을 반영해 해당 항목을 재분석하거나 수정한 뒤 개발한다.
- 채팅 응답도 같은 규칙으로 처리한다.
- Slack MCP가 사용할 수 없으면 사용자에게 Slack 연결이 필요하다고 알리고 채팅 승인 흐름은 계속 진행한다.

### SmartContract와 HTML 자동 동기화

- SmartContract에 외부에서 호출 가능한 `public` 또는 `external` 함수가 추가, 변경, 삭제되면 Blockchain-API 연동 영향으로 간주한다.
- `smartcontract/src/Payment.sol`에 호출 가능한 함수가 추가될 때마다 `blockchain-api/src/server.js`에 대응 JSON-RPC API 메서드를 추가한다.
- 함수 ABI, 이벤트, 에러, 배포 주소가 바뀌면 `blockchain-api/deployments/payment.json`, `blockchain-api/src/paymentAbi.js`, `blockchain-api/src/server.js` 반영 여부를 확인한다.
- 새로 추가된 컨트랙트 호출 함수가 사용자가 직접 실행할 수 있는 기능이면 `blockchain-api/public/index.html`에 해당 입력 폼 또는 버튼을 자동으로 추가한다.
- HTML을 수정할 때 필요한 경우 `blockchain-api/public/app.js`와 `blockchain-api/public/styles.css`도 함께 수정해 실제 호출 흐름이 동작하게 한다.
- HTML은 사용자가 기능을 직관적으로 알 수 있게 구성한다. 버튼/폼 라벨은 컨트랙트 함수명을 그대로 노출하기보다 사용자가 수행할 동작을 설명해야 하며, 입력 필드는 토큰 주소, 가맹점 EOA, 수량처럼 도메인 용어로 표시한다.
- 컨트랙트 함수가 아직 없거나 제거된 경우에는 해당 API와 HTML 기능도 제거하고, 화면에는 호출 가능한 기능이 없다는 상태를 명확히 보여준다.
- 이 HTML/API 동기화는 별도 승인 항목이 없어도 해당 SmartContract 변경의 필수 후속 작업으로 처리한다. 단, 기능 범위가 PRD를 넘어서는 새 정책 판단이 필요하면 사용자에게 확인한다.
- 프론트엔드에 노출하는 호출명, 입력값, 출력값은 컨트랙트 ABI와 JSON-RPC 메서드명에 맞춘다.

### 구현 및 검증

- `Payment.sol`에 기능을 추가하거나 변경할 때마다 기본적으로 `smartcontract/test/Payment.t.sol`에 대응 테스트를 작성하거나 갱신한다.
- `blockchain-api`에 API 또는 프론트엔드 기능을 추가하거나 변경할 때마다 기본적으로 가능한 범위의 테스트 코드 또는 검증 스크립트를 작성하거나 갱신한다.
- 테스트 작성이 불가능한 환경이면 이유를 명확히 기록하고, 최소한 문법 검사, JSON 파싱, 정적 검색, 수동 검증 절차 중 가능한 검증을 수행한다.
- SmartContract 변경 후에는 가능한 범위에서 `smartcontract` 테스트를 실행한다.
- Blockchain-API 변경 후에는 가능한 범위에서 API 또는 프론트엔드 동작을 확인한다.
- ABI, 배포 주소, 컨트랙트 이벤트/함수 시그니처가 바뀌면 SmartContract와 Blockchain-API 양쪽 반영 여부를 확인한다.
- 보안 관련 변경은 권한, 재진입, 입력 검증, 이벤트 로그, 상태 변경 순서를 검토한다.
- 리스트 항목을 구현한 뒤 사용자 채팅과 Slack에 결과를 알릴 때는 항상 해당 항목의 테스트/검증 결과를 함께 포함한다.
- 테스트가 실패했거나 실행되지 못한 경우에도 숨기지 않고 명령, 실패 사유, 남은 위험을 함께 보고한다.

### 최종 리포트

승인된 모든 항목 개발이 완료되면 최종 리포트를 작성하고 Slack에 알린다.
사용자가 list 응답 개수 자체를 개발 완료 조건으로 명시한 경우에도 최종 리포트는 자동 작성한다.

최종 리포트 형식:

```text
# {{PRD_pageName}} {{version}} 최종 리포트

## 완료 항목
- {{번호}}. {{요약}}

## 스킵 항목
- {{번호}}. {{요약 및 사유}}

## 테스트/검증
- {{실행한 명령 및 결과}}

## 보안성 검토
- {{검토 결과}}

응답: 1:확인, 2:코멘트
```

최종 리포트 응답 규칙:

- `1:확인`이면 최종 보안성 검토를 한 번 더 수행하고 완료 처리한다.
- `2:코멘트`이면 코멘트에 해당하는 리스트 항목을 다시 수정한다.
- 최종 리포트의 `1`, `2` 응답도 사용자 채팅과 Slack thread 양쪽에서 받을 수 있다.
- 최종 리포트 확인 질문도 현재 Codex 채팅창과 Slack에 동시에 알리고 최대 1분 동안 응답을 기다린다. 1분 안에 유효 응답이 없으면 완료 처리나 다음 단계로 진행하지 않는다.

### 배포 명령

리스트 전체 개수와 `확인` 처리된 개수가 일치하면 `smartcontract` 폴더에서 다음 명령을 실행한다.
사용자가 list 응답 개수 자체를 개발 완료 조건으로 명시한 경우에도 배포 명령은 자동 실행한다.

```bash
node scripts/deploy-payment.mjs
```

명령 완료 후 사용자 채팅과 Slack에 GitHub Push 여부를 묻는다.
사용자가 list 응답 개수 자체를 개발 완료 조건으로 명시한 경우에도 GitHub Push 여부를 자동으로 묻는다.

```text
배포 명령이 완료되었습니다. GitHub에 Push할까요?
응답: 1:진행, 2:스킵
```

- `1:진행`이면 변경사항 확인 후 커밋/푸시 절차를 진행한다.
- `2:스킵`이면 푸시하지 않고 현재 상태를 보고한다.
- 배포와 Push 확인 응답도 사용자 채팅과 Slack thread 양쪽에서 받을 수 있다.
- 배포와 Push 확인 질문도 현재 Codex 채팅창과 Slack에 동시에 알리고 최대 1분 동안 응답을 기다린다. 1분 안에 유효 응답이 없으면 Push 또는 후속 질문으로 진행하지 않는다.

### 필수 작업

1. Jira 관련 요청은 Jira MCP를 활용한다.
